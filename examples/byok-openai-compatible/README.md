...
1. Copy `.env.example` to `.env` and fill in your keys. Analysis / Runtime load env from the workspace `.env`.
...
7. Click **一键复刻** → generates `productions/replica-*/` with:
   ...
   - **B-roll**: `gpt:Image` with `<gpt:Reference image={product-reference}/>`
...
- H3 ReferenceVideo does **not** use MiniMax `/files`. Set `referenceUpload` to `s3`; MiniMax
  fetches `{S3_CDN}/hypit/references/…`.
- H3 is bound as `@hypit/minimax-h3@1#minimax-h3` → `MiniMax-H3` on `gateway.minimax`
  (`H3_VIDEO_API_KEY`, `H3_VIDEO_BASE_URL`).
- H3 clips are capped at 15 seconds per request; longer references are split into timeline shots.
